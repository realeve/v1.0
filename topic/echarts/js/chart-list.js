...
$(document).ready(function() {
	...
	for (var e = 0, l = EXAMPLES.length; l > e; ++e) {
		...
		$("#chart-row-" + EXAMPLES[e].type).append(c.append(n)), $link = $('<a class="chart-link" href="demo.html#' + EXAMPLES[e].id + '"></a>'), n.append($link), $link.append($('<div class="chart-hover"></div>').append('<h4 class="chart-title">' + i + "</h4>").append('<div class="chart-subtitle">' + r + "</div>")).append('<div class="chart-hover-bg"></div>'), $chartArea = $('<img class="chart-area" data-original="' + GALLERY_THUMB_PATH + EXAMPLES[e].id + '.png" />'), $link.append($chartArea)
	}
