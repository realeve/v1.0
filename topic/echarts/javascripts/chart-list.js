...
$(document).ready(function() {
	...
	for (var i = 0, l = EXAMPLES.length; l > i; ++i) {
		var r = EXAMPLES[i].title || "未命名图表",
			p = EXAMPLES[i].subtitle || "点击查看详情",
			d = $('<div class="col-lg-3 col-md-4 col-sm-6"></div>'),
			n = $('<div class="chart"></div>');
		$("#chart-row-" + EXAMPLES[i].type).append(d.append(n)), $link = $('<a class="chart-link" href="./editor.html?c=' + EXAMPLES[i].id + '"></a>'), n.append($link), $link.append($('<div class="chart-hover"></div>').append('<h4 class="chart-title">' + r + "</h4>").append('<div class="chart-subtitle">' + p + "</div>")).append('<div class="chart-hover-bg"></div>'), $chartArea = $('<img class="chart-area" src="data/thumb/' + EXAMPLES[i].id + '.png" />'), $link.append($chartArea)
	}
