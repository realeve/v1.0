...
require(['jquery', 'jquery.fullPage', 'jquery-weui'], function($) {
	...
	var handleAnswer = function() {
		...
		function handleTotalScore(iScore, uid) {
			...
			$('[name="sucessInfo"] .weui_msg_desc').html('您一共得了<span name="totalScore" style="font-weight:bold;color:#445"> ' + iScore + ' </span>分');
			...
			if (!validate(data)) {
				...
				$.ajax({
						...
						success: function(obj) {
							if (obj.status == 0) {
								$('[name="sucessInfo"] .weui_msg_title').text('提交失败，请裁屏保存分数或稍后重试');
