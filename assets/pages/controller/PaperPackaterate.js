var PaperValidate = function() {
  ...
  function getSelectInfo() {
    ...
    var str = getRootPath(1) + "/assets/pages/controller/data/paper_validate.json";
    $.ajax({
        url: str
      })
      .done(function(data) {
        ...
        $("select[name='reel_end']").html(str.join(''));

        $("select[name='suspect_paper']").html(str.join(''));

        $("select[name='well_paper']").html(str.join(''));
      });
    initSelect2();
  }
  ...
  function vialidate() {
    if ($('select[name="prod_id"]').val() === '' || $('select[name="machine_id"]').val() === '') {
      return 1;
    }
    return 0;
  }
  ...
  function loadHisData() {
    ...
    if (Data.data[0].rec_date > "2017-04-26") {
      SetSelect2Val('reel_end', Data.data[0].reel_end);
      SetSelect2Val('suspect_paper', Data.data[0].suspect_paper);
      SetSelect2Val('well_paper', Data.data[0].well_paper);
    }
