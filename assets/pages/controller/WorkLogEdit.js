function WorkLogInit() {
  ...
  $('#ProUserName').multiSelect({
    keeporder: true,
    selectableHeader: "<div class='custom-header'>可选名单</div>",
    selectionHeader: "<div class='custom-header'>已选择</div>",
    selectableOptgroup: true,
    //此处记录数据选择情况
    afterSelect: function(values) {
      for (var i = values.length - 1; i >= 0; i--) {
        var iIndex = values[i]; //取得值
        iProUserName[iIndex] = 1; //选择
      }
    },
    afterDeselect: function(values) {
      for (var i = values.length - 1; i >= 0; i--) {
        var iIndex = values[i]; //取得值
        iProUserName[iIndex] = 0;
      }
    }
  });

  $('#TransUserName').multiSelect({
    keeporder: true,
    selectableHeader: "<div class='custom-header'>可选名单</div>",
    selectionHeader: "<div class='custom-header'>已选择</div>",
    selectableOptgroup: true,
    ...
    afterSelect: function(values) {
      for (var i = values.length - 1; i >= 0; i--) {
        var iIndex = values[i]; //取得值
        iTransUserName[iIndex] = 1; //选择
      }
    },
    afterDeselect: function(values) {
      for (var i = values.length - 1; i >= 0; i--) {
        var iIndex = values[i]; //取得值
        iTransUserName[iIndex] = 0;
      }
    }
  });
}
