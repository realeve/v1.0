$(document).ready(function() {
  ...
  var cart_number = getUrlParam('cart');

  var loadData = function() {
    $('h1').html('<a href="http://10.8.2.133/search/#' + cart_number + '" target="_blank" alt="点击查看生产详情">' + cart_number + '</a>' + ' 机台连续废通知记录');

    $.ajax({
        url: 'http://10.8.2.133/datainterface/api?ID=538&cart=' + cart_number,
      })
      .done(function(res) {
        res = JSON.parse(res);
        res.header = [{ title: '序号' }].concat(res.header);
        var headHtml = res.header.map(function(item) {
          return '<th>' + item.title + '</th>'
        })
        var trHtml = res.data.map(function(tr, i) {
          var tdHtml = tr.map(function(td) {
            return '<td>' + td + '</td>'
          })
          return '<tr>' + ['<td>' + (i + 1) +
            '</td>'
          ].concat(tdHtml).join('') + '</tr>'
        })
        $('tbody').html(trHtml.join(''));
        $('table thead,table tfoot').html('<tr>' + headHtml.join('') + '</tr>');
