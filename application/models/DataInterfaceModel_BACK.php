...
class DataInterfaceModel extends CI_Model
{
    ...
    public $DBLIST = ['Quality', 'XZHC', 'QFM', 'JTZY', 'KG', 'sqlsvr', 'SHY', 'ZXQS', 'OFFICEHELPER', 'CZUSER', 'IMG'];
    ...
    public function Api($data)
    {
        ...
        //先获取当前用户ID
        $LOGINDB['sqlsvr'] = $this->load->database('sqlsvr', true);
        //$LOGINDB->cache_off();

        //缓存中没有则往缓存写数据
        //如果API更新，需将相应数据删除
        $key = 'sql_id_' . $data['ID'] . '_token_' . $data['Token'];
        if (!$strJson = $this->cache->get($key)) {
            ...
            $query   = $LOGINDB['sqlsvr']->query($SQLStr, array($data['Token']));
            $strJson = $query->result_json();
...
//当参数为空时
    ...
    public function ShowApiData($aParams, $ApiInfo, $mode, $dataParams)
    {
        ...
        $LOGINDB = $this->load->database($this->DBLIST[$ApiInfo->DBID], true);
